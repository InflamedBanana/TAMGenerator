...
void Map::Resize(const float _resizeValue, const ci::CImg<unsigned char>& _higherImg)
{
	...
	ci::CImg<unsigned char> higherImg(_higherImg, false);

	ci::CImg<unsigned char>* lowerImg = new ci::CImg<unsigned char>();

	lowerImg->assign(newMapSize, newMapSize, 1, 1);

	higherImg = higherImg.get_resize(_resizeValue * -100, _resizeValue * -100, -100, -100, 3);
	...
	cout << "1 : Map size : " << m_size << " | lower img size : " << lowerImg->width() << " | lower img data size : " << lowerImg->size() << endl;

	m_img = lowerImg;
	m_size *= _resizeValue;
	cout << "2 : Map size : " << m_size << " | lower img size : " << lowerImg->width() << " | lower img data size : " << lowerImg->size() << endl;
}
...
void Tone::ComputeLowerMipMaps()
{
	for (int i = 3; i >= 0; i --)
	{
		...
		/*ci::CImg<unsigned char> lowerImg, higherImg(m_maps[4].img());
		lowerImg.assign(mapSize, mapSize, 1, 1);*/
		

		lower.Resize(1/pow(2,4-i), m_maps[4].img());

		cout << "3 : Map size : " << m_maps[i].size() << " | img data size : " << m_maps[i].img().size() << endl;
